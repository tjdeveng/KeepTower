...
#include "MultiUserTypesSerDeDetail.h"
...
std::optional<PasswordHistoryEntry> PasswordHistoryEntry::deserialize(
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, SERIALIZED_SIZE,
...
std::optional<VaultSecurityPolicy> VaultSecurityPolicy::deserialize(const std::vector<uint8_t>& data) {
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 4, "VaultSecurityPolicy: pbkdf2_iterations")) {
        return std::nullopt;
    }
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 4, "VaultSecurityPolicy: password_history_depth")) {
        return std::nullopt;
    }
    ...
    if (data.size() >= MID_V2_SIZE) {
        ...
        if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 1, "VaultSecurityPolicy: username_hash_algorithm")) {
            return std::nullopt;
        }
    ...
    if (data.size() >= CURRENT_V2_SIZE) {
        ...
        if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 4, "VaultSecurityPolicy: argon2_memory_kb")) {
            return std::nullopt;
        }
        ...
        if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 4, "VaultSecurityPolicy: argon2_iterations")) {
            return std::nullopt;
        }
        ...
        if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 1, "VaultSecurityPolicy: argon2_parallelism")) {
            return std::nullopt;
        }
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 64, "VaultSecurityPolicy: yubikey_challenge")) {
        return std::nullopt;
    }
    ...
    if (data.size() >= MIGRATION_V2_SIZE) {
        ...
        if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 1, "VaultSecurityPolicy: username_hash_algorithm_previous")) {
            return std::nullopt;
        }
        ...
        // Bytes 89-96: migration_started_at (big-endian uint64_t)
        if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 8, "VaultSecurityPolicy: migration_started_at")) {
            return std::nullopt;
        }
        ...
        // Byte 97: migration_flags
        if (!MultiUserTypesSerDeDetail::require_bytes(data, offset, 1, "VaultSecurityPolicy: migration_flags")) {
            return std::nullopt;
        }
...
std::optional<std::pair<KeySlot, size_t>> KeySlot::deserialize(
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, pos, 64, "KeySlot: username_hash")) {
        return std::nullopt;
    }
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, pos, 1, "KeySlot: username_hash_size")) {
        return std::nullopt;
    }
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, pos, 16, "KeySlot: username_salt")) {
        return std::nullopt;
    }
    ...
    if (!MultiUserTypesSerDeDetail::require_bytes(data, pos, 32 + 40 + 1 + 1 + 8 + 8, "KeySlot: core fields")) {
        return std::nullopt;
    }
...
std::optional<VaultHeaderV2> VaultHeaderV2::deserialize(const std::vector<uint8_t>& data) {
    if (!MultiUserTypesSerDeDetail::require_bytes(data, 0, VaultSecurityPolicy::SERIALIZED_SIZE + 1,
                                                 "VaultHeaderV2")) {
        return std::nullopt;
    }
