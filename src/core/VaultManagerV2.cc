...
KeepTower::VaultResult<KeepTower::UserSession> VaultManager::open_vault_v2(
    ...
    if (m_vault_data.backup_count() > 0) {
        set_backup_enabled(m_vault_data.backup_enabled());
        set_backup_count(m_vault_data.backup_count());
        Log::debug(
...
KeepTower::VaultResult<> VaultManager::migrate_user_hash(
    ...
    Log::info("VaultManager: Saving vault after migration (creating backup)");
    if (!save_vault(true)) {  // true = create backup
